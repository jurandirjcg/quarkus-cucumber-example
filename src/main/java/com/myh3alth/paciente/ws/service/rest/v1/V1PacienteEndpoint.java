...
//Fault Tolerance
//@Retry(
//    maxRetries = Utils.DEFAULT_RETRY_MAX_RETRIES)
...
@Tag(
    name = "Pacientes")
public class V1PacienteEndpoint {
    ...
    @Operation(
        summary = "salvar os dados do paciente")
    @APIResponse(
        content = @Content(
            schema = @Schema(
                implementation = V1ResponsePaciente.class)))
    public V1ResponsePaciente salvar(@RequestBody @Valid final V1RequestPaciente paciente) throws SaveEntityException {
     ...
     *
     * @param id     {@link Long}
     * @param fields {@link WSFieldParam}
     * @return {@link V1ResponsePaciente}
    ...
    @LinkResources(
        value = { @LinkResource(
            ...
            @LinkResource(
                ...
                pathParameters = { "" }) })
    @Operation(
        summary = "obtém as informações do paciente")
    @APIResponse(
        content = @Content(
            schema = @Schema(
                implementation = V1ResponsePaciente.class)))
    public V1ResponsePaciente obterPaciente(
        @Parameter() @NotNull @PathParam("id") final Long id,
        @Parameter(
            schema = @Schema(
                implementation = String.class)) @QueryParam("fields") @DefaultValue("id,nome") @WSParamFormat(V1ResponsePaciente.class) final WSFieldParam fields)
     ...
     * @param dataNascimento {@link String}
     * @param page           {@link Integer}
     * @param limit          {@link Integer}
     * @param fields         {@link WSFieldParam}
     * @param sort           {@link WSSortParam}
     * @return {@link Page}
     * @throws ApplicationException {@link ApplicationException}
     */
    @GET
    ...
    @LinkPaginate(
        ...
        embeddedCollectionName = "pacientes")
    @Operation(
        summary = "paginação de pacientes conforme filtros aplicados")
    @APIResponse(
        content = @Content(
            schema = @Schema(
                implementation = V1ResponsePaciente.class)))
    public Page<V1ResponsePaciente> paginarPaciente(
        ...
        @Parameter(
            description = "Formato yyyy-mm-dd. Permite busca através de expressões regulares: <, <=, =, !=, >=, > e btwn. Ex: 2001-01-20 btwn 2010-10-25") @QueryParam("dataNascimento") final String dataNascimento,
        @Parameter(
            schema = @Schema(
                implementation = String.class,
                defaultValue = "id,nome")) @QueryParam("fields") @DefaultValue("id,nome") @WSParamFormat(V1ResponsePaciente.class) final WSFieldParam fields,
        @Parameter(
            schema = @Schema(
                implementation = String.class,
                defaultValue = "nome:asc")) @QueryParam("sort") @DefaultValue("nome:asc") @WSParamFormat(V1ResponsePaciente.class) final WSSortParam sort,
        @Parameter(
            schema = @Schema(
                defaultValue = Utils.DEFAULT_REST_PAGE)) @QueryParam("page") @DefaultValue(Utils.DEFAULT_REST_PAGE) final Integer page,
        @Parameter(
            schema = @Schema(
                defaultValue = Utils.DEFAULT_REST_LIMIT)) @QueryParam("limit") @DefaultValue(Utils.DEFAULT_REST_LIMIT) final Integer limit)
        throws ApplicationException {
